"""Build hard confusion-count matrices from few-shot training images."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build hard confusion-count matrices from few-shot train_x"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="one or more shot counts, for example: --shots 4 8 16 32",
    )
    parser.add_argument("--seed", type=int, default=1, help="few-shot sampling seed")
    parser.add_argument("--output", default=".", help="output root directory")
    return parser.parse_args()


def build_cfg(args, shots):
    cfg = get_cfg_default()
...
def save_figure(matrix, classnames, path, shots, seed):
    ...
    axis.set_title(f"Few-shot Confusion Counts ({shots}-shot, seed {seed})")
...
def main():
    args = parse_args()
    shots_list = list(dict.fromkeys(args.shots))
    if any(shots < 1 for shots in shots_list):
        raise ValueError("Every value passed to --shots must be at least 1")

    set_random_seed(args.seed)
    first_dataset = build_dataset(build_cfg(args, shots_list[0]))
    classnames = first_dataset.classnames
    ...
    for index, shots in enumerate(shots_list):
        if index == 0:
            dataset = first_dataset
        else:
            set_random_seed(args.seed)
            dataset = build_dataset(build_cfg(args, shots))
        if dataset.classnames != classnames:
            raise RuntimeError("Class names changed between shot settings")

        loader = DataLoader(
            TrainImages(dataset.train_x, preprocess),
            batch_size=BATCH_SIZE,
            shuffle=False,
            num_workers=0,
        ...
        matrix = matrix.cpu()
        output_dir = Path(args.output) / f"shots_{shots}" / f"seed_{args.seed}"
        output_dir.mkdir(parents=True, exist_ok=True)
        matrix_path = output_dir / "confusion_count_matrix.pt"
        figure_path = output_dir / "confusion_count_matrix.png"
        torch.save(matrix, matrix_path)
        save_figure(matrix, classnames, figure_path, shots, args.seed)

        print(f"Class names ({shots}-shot, seed {args.seed}):")
        for class_index, classname in enumerate(classnames):
            print(f"  {class_index}: {classname}")
        torch.set_printoptions(profile="full", linewidth=200)
        print("Confusion count matrix [true, predicted]:")
        print(matrix)
